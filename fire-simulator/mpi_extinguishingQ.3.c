 ...
 */ 
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		MPI_Allreduce(&local_num_deactivated, &num_deactivated, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
		...
		for( step=0; step<10; step++ )	{
			...
			if ( rank > 0 ) {
				...
				/* Rank 0: top halo (row 0) corresponds to global border row - keep zeros or existing values */
			}
			/* Exchange with bottom neighbor (rank+1): send local row chunk, receive into row chunk+1 */
			if ( rank < size-1 ) {
			...
			/* We update only local real rows (1..chunk) whose global index is in [1 .. global_rows-2] */
			for( i=1; i<=chunk; i++ ) {
				int gx = g_start + (i-1);
				...
				for( j=1; j<columns-1; j++ ) {
					accessMat( surface, i, j ) = (
						...
						accessMat( surfaceCopy, i, j+1 ) ) / 4.0f;
				}
			}
				...
				for( j=teams[t].y-radius; j<=teams[t].y+radius; j++ ) {
					...
					if ( distance <= radius ) {
						...
						if ( i >= g_start && i <= g_end ) {
							...
							accessMat( surface, local_i, j ) = accessMat( surface, local_i, j ) * ( 1 - 0.25 ); // Team efficiency factor
						}
					}
				}
			}
	...
	/* After simulation, gather the full surface into rank 0 so the remaining (sequential) code can print results */
	...
	MPI_Gather( &accessMat(surface,1,0), chunk*columns, MPI_FLOAT,
				fullSurface, chunk*columns, MPI_FLOAT,
				0, MPI_COMM_WORLD );

	/* Replace local pointer 'surface' on rank 0 to point to fullSurface for the printing section below */
	if ( rank == 0 ) {
